...
contract EventRewardManager is Ownable {
    ...
    external
  {
    ...
    require(token.transferFrom(_creator, address(this), _rewardAmount), "Token transfer failed");
    ...
    emit TokenRewardCreated(_eventId, _creator, _tokenAddress, _tokenType, _rewardAmount);
  }

  // Update token-based event reward amount
  function updateTokenReward(address _eventManager, uint256 _eventId, uint256 _amount) external {
    checkZeroAddress();
    ...
    require(token.transferFrom(_eventManager, address(this), _amount), "Token transfer failed");
  ...
  function claimTokenReward(uint256 _eventId, address _participant) external {
    ...
    require(!hasClaimedTokenReward[_eventId][_participant], "Reward already claimed");
    ...
    IERC20 token = IERC20(eventReward.tokenAddress);
    require(token.transfer(_participant, rewardAmount), "Token transfer failed");
  ...
  function withdrawUnclaimedRewards(uint256 _eventId, address _eventManager) external {
    checkZeroAddress();
    checkEventIsValid(_eventId);
    ...
    IERC20 token = IERC20(eventReward.tokenAddress);
    require(token.transfer(_eventManager, remainingReward), "Token withdrawal failed");

    emit TokenRewardWithdrawn(_eventId, _eventManager, remainingReward, cancelled);
  }
}
