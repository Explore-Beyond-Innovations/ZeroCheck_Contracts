...
contract EventRewardManager is Ownable, IEventRewardManager {
    ...
    event TokenRewardBonusDistributed(uint256 indexed eventId, address indexed recipient, uint256 bonus);

    event MultipleTokenRewardDistributed(uint256 indexed eventId, address[] indexed recipients, uint256[] amounts);
    ...
    function createNFTReward(
    ...
    ) external onlyEventContract checkEventCreator(_eventId, creator) {
        checkZeroAddress();
        ...
        EventNFT nft = new EventNFT(_name, _symbol, _maxSupply, _baseURI, _BonusbaseURI, address(eventManager));
    ...
    function updateTokenReward(uint256 _eventId, address eventCreator, uint256 _amount)
        external
        onlyEventContract
    ...
    function distributeTokenReward(uint256 _eventId, address _creator, address _recipient, uint256 _participantReward)
        external
        onlyEventContract
    ...
    ) external onlyEventContract checkEventCreator(_eventId, _creator) {
        checkEventIsValid(_eventId);
    ...
    function getMultipleDistributedTokenRewards(uint256 _eventId, address[] calldata _participants)
        external
        view
    ...
    function setFirstParticipantTokenBonus(uint256 _eventId, address _eventCreator, address _recipient, uint256 _bonus)
        external
    ...
    function withdrawUnclaimedRewards(uint256 _eventId, address creator)
        external
        onlyEventContract
