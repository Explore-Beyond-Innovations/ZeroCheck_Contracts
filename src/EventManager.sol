...
contract EventManager is Ownable, IEventManager {
    ...
    event ParticipantRewardSet(
        uint256 indexed eventId, address indexed participant, address indexed tokenAddress, uint256 amount
    );
    event BulkTokenRewardSet(uint256 indexed eventId, address[] indexed recipients, uint256[] amounts);
    event FirstParticpantBonusSet(uint256 indexed eventId, address indexed participant, uint256 bonus);
    ...
    constructor(address _worldId, uint256 _root, string memory _appId, string memory _actionId, uint256 _groupId)
        Ownable(msg.sender)
    {
    ...
    function registerParticipant(uint256 eventId, uint256 nullifierHash, uint256[8] calldata proof)
        public
        isNotEnded(eventId)
    {
    ...
    function createReward(
    ...
    ) external onlyEventCreator(_eventId) {
        ...
        if (_tokenType == IEventRewardManager.TokenType.NFT) {
            rewardManager.createNFTReward(
                _eventId, msg.sender, _tokenType, _tokenName, _tokenSymbol, _maxSupply, _baseURI, _BonusbaseURI
            );
        } else {
            rewardManager.createTokenReward(_eventId, msg.sender, _tokenType, _tokenAddress, _rewardAmount);
        }

        events[_eventId].rewardSet = true;
    }

    function updateEventTokenReward(uint256 _eventId, uint256 _amount)
        external
    ...
    function setTokenRewardForParticipant(uint256 eventId, address _participant, uint256 _reward)
        external
    ...
    function claimReward(uint256 eventId, uint256 nullifierHash, uint256[8] calldata proof)
        external
        isNotEnded(eventId)
    {
    ...
    function claimNFTReward(uint256 eventId, uint256 nullifierHash, uint256[8] calldata proof)
        external
        isNotEnded(eventId)
    {
        require(msg.sender != address(0), "Invalid address");
    ...
    function _verifyProof(uint256 nullifierHash, uint256[8] calldata proof) internal view {
        worldId.verifyProof(
            worldIdRoot, groupId, abi.encodePacked(msg.sender).hashToField(), nullifierHash, externalNullifier, proof
        );
    }
}
